...
\author{Susana Bejar\and Diane Massam\and Ana-Teresa Pérez-Leroux\lastand Yves
Roberge\affiliation{University of Toronto}}
\title{Rethinking complexity}

% \chapterDOI{} %will be filled in at production

\abstract{This paper addresses the nature of complexity of \isi{recursion}. We
    consider four asymmetries involving caps on \isi{recursion} observed in previous
    experimental acquisition\is{language acquisition} studies, which argue that complexity cannot be
    characterized exclusively in terms of the number of iterations of \isi{Merge}.
While \isi{recursion} is essentially syntactic and allowed for by the minimalist
toolkit via \isi{Merge}, selection, and labeling\is{labelling} or projection, the complexity of
...
equally complex.  This squib sheds light on the nature of the \isi{complexity} of
recursion in human grammar through a theoretically-based exploration of four
asymmetries observed in a series of experimental studies on the acquisition\is{language acquisition} of
self-embedding structures we have conducted in the last few years. Note that,
while \enquote{self-embedding} often refers to complement structures, our use
of the term generalizes over \isi{adjunction} as well.
...
referential task, we elicited twice-embedded genitives\is{genitive case} \REF{ex:key:1a} and
modificational PPs \REF{ex:key:1b}. Contexts were set up so twice-embedded
modification was needed to disambiguate target referents from other competing
referents. For instance, we need something like \REF{ex:key:1b} to uniquely
...
\isi{coordination}, as in \REF{ex:key:2}, which matched the utterance length of
...
\REF{ex:key:1}. This precludes a purely structural explanation of the relative
...
\REF{ex:key:3a}, whereas in the other \REF{ex:key:3b}, the head noun is
...
\REF{ex:key:3a} and \REF{ex:key:3b} are equivalent with respect to the number
of phasal domains (assuming one views DP as a phase\is{phases}), in
\REF{ex:key:3b}, but not \REF{ex:key:3a}, the referent of the head noun is
...
phase\is{phases} \emph{in the water} in \REF{ex:key:3b} was essential for
...
\REF{ex:key:4a}, speakers commonly substituted \isi{relative clauses}
\REF{ex:key:4b} and a mix of PP and relative clause\is{relative clauses}
constructions \REF{ex:key:4c}.
...
embedding \REF{ex:key:1a}, which differs from comitative PPs \REF{ex:key:1b} in
