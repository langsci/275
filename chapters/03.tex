...
\ea\label{ex:key:3.2} Word order parameter hierarchy\is{parameter hierarchies} \citep{Roberts2012}:
...
represented in \eqref{ex:key:3.5}.  Parameters in this system are thus an
...
\begin{exe}
    \ex\label{bkm:Ref347566517}\label{ex:key:3.5}%
...
This NONE > ALL > SOME acquisition\is{language acquisition} creates a hierarchy that we can think of as
...
In a parameter hierarchy\is{parameter hierarchies} for word order as in \eqref{ex:key:3.2} above, the \ili{Bantu}
...
Two examples are O-V order in \ili{Tunen}, and final question particles in languages
like \ili{Rangi}\footnote{Rangi (like some surrounding languages) is famous for its
...
the auxiliary\is{auxiliaries} argues against roll-up movement,\is{roll-up movement} and thus
against an analysis as involving the same feature. Furthermore, the strict
adjacency required between the infinitival verb and the auxiliary\is{auxiliaries}, as well as
the fact that clauses with a filled C-domain (relative, cleft,\is{clefts} wh, \isi{focus})
require aux-V order,  argues in favour of V-aux as a derived by phrasal
movement\is{movement} of only the infinitive to the specifier of the aux,
rather than a full comp-to-spec movement.} and \ili{Zulu}.  While these languages are
...
\ea\label{bkm:Ref373494395}\label{ex:key:3.7}\ili{Tunen} (A44, \citealt[126]{Mous1997})
    \ea
    \gll 	\`{A}ná  mònɛ  índì.\\
            \Tsg{}.\Pst{}  money  give\\
...
\ea\label{ex:key:3.8}\ili{Zulu} (S42, \citealt[69]{Buell2005})
	\ea
...
\z
\newpage
\ea \ili{Rangi} (F33, \citealt[51]{Gibson2012})
    \ea
...
\Cref{fig:key:1} .\footnote{Some \ili{Bantu} languages also use \gls{TAM} inflections
...
\begin{figure}[hbt]
    ...
    \end{tabularx}
    \end{small}
    \caption{Slots in the \ili{Bantu} verb}\label{fig:key:1}
\end{figure}
...
\ea\label{ex:key:3.10} \ili{Makhuwa} (P31, \citealt[168--169]{vanderWal2009})
    \sn
...
\ea\label{ex:key:3.12} \ili{Makhuwa} (P31, \Citealt[169]{vanderWal2009})
    \sn
...
\ea\label{ex:key:3.13} \ili{French}\\
    \sn
...
and \eqref{ex:key:3.17}.\glsunset{BEN}\glsunset{TH}
...
allowing \isi{object marking} of the causee and not the theme \eqref{ex:key:3.20}.

\ea\label{ex:key:3.19}\ili{Otjiherero} (R30, \citealt[247]{MartenKula2012})
    \sn Applicative
	\ea
...
\ea\label{ex:key:3.20}\ili{Otjiherero} (R30, Jekura Kavari, personal communication)
    \sn Causative
	\ea
...
different heights of causatives). The pattern in \tabref{tab:key:1} can then be
...
\ea \Citeauthor{vanderWal2017} (\citeyear{vanderWal2017}, adapted)
    \sn Parameter hierarchy\is{parameter hierarchies} for (a)symmetry in
    ditransitive \isi{alignment}
        \begin{tikzpicture}[baseline]
...
\eqref{ex:key:3.23}.%\largerpage[2]

\ea\label{ex:key:3.22} \ili{Chichewa} (N31)
	\ea
...
\ea\label{ex:key:3.25} \ili{Luganda} (JE15, \citealt[67, 72]{Ssekiryango2006})\label{bkm:Ref286389666}
    \ea
...
\ea\label{ex:key:3.26}\label{bkm:Ref347511825}
    \begin{tikzpicture}[baseline=-.79\baselineskip]
...
\eqref{ex:key:3.27}, following the now-familiar none--all--some
sequence.\newpage%\largerpage[1]

\ea\label{bkm:Ref255201037}\label{ex:key:3.27} Possible uφ feature\is{φ-features} hierarchy 1
...
on T) and agreeing relative complementisers\is{complementizers} (${\varphi}$ on C). Object and
...
\ea\label{ex:key:3.28} \ili{Ciluba} Kasai (L31, \citealt[87]{Cocchi2000})\label{bkm:Ref345186241}
    \sn
...
\ea\label{ex:key:3.29} \ili{Ciluba} Kasai (L31, \citealt[104]{deKindBostoen2012}) \label{bkm:Ref345186270}
	\ea
...
relative \isi{agreement} for ${\varphi}$ features: the relative clause in
...
\ea\label{ex:key:3.32} \ili{Makhuwa} (P31) \label{bkm:Ref347928062}
    \sn
...
\ea\label{ex:key:3.35} \ili{Basaa} (A43, \citealt[278]{Hyman2003})\label{bkm:Ref347562250}
    \ea
...
\ea\label{ex:key:3.36} \ili{Basaa} (A43, \citealt[19, 20]{JenksEtAl2017}) \label{bkm:Ref347562274}
    \ea
...
construction, then \ili{Basaa} can be taken to illustrate a language in which only T
has uφ features\is{φ-features}, whereas C, v and Appl do not.

The parameter hierarchy\is{parameter hierarchies} for \ili{Bantu} languages discussed so far would thus come
out as follows:
...
\ea\label{ex:key:3.39} \ili{Bembe} (D54, \citealt[103]{Iorio2014}) \label{bkm:Ref298742208}
	\ea
...
\ea\label{ex:key:3.40} \ili{Bembe} (D54, \citealt[152]{Iorio2014}) \label{bkm:Ref347515914}
	\ea
...
This cross-linguistic situation, as illustrated in~\Cref{tab:key:2}, suggests
...
The separate \isi{parameters} can then can be modelled as in \eqref{ex:key:3.43},
...
\ea\label{ex:key:3.44} \ili{Luguru} (G35, \citealt[264--265]{MartenRamadhani2001})
...
\z
\newpage
\ea\label{ex:key:3.45} \ili{Luguru} (G35, \citealt[264--265]{MartenRamadhani2001})
...
\ea\label{ex:key:3.46} \ili{Luguru} (G35, \citealt[264--265]{MartenRamadhani2001}) \label{bkm:Ref347659685}
    \sn
...
\ea\label{ex:key:3.47} \ili{Luguru} (G35, \citealt[265--266]{MartenRamadhani2001})
    ...
    \ex
    \gll    \llap{?}Wanzehe  wa-pfika  ipfitabu.\\
...
\ea\label{ex:key:3.48}\label{bkm:Ref348109619}%\vspace{-1\baselineskip}
