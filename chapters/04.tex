...
\blockquote{In the absence of a list, however tentative, of posited \isi{parameters} and their
...
pronouns. For general discussion of empty elements, see \textcite[Ch.\
...
individual languages and for how we see differences between languages.

For \gls{HPSG}\is{Head-Driven Phrase Structure Grammar}, a linguistic analysis is a system of types, features, and
constraints.\footnote{The related but slightly different framework,
Sign-Based Construction Grammar, has a further major element, namely
constructions.\glsunset{SBCG} For \gls{SBCG} signs are defined in terms of
constraints on constructions, whereas standard \gls{HPSG}\is{Head-Driven Phrase
Structure Grammar} has constraints applying directly to signs. \gls{SBCG} is
more complex in some respects but simpler in others. In particular, it has a
simpler notion of sign and is able to dispense with a number of features and
types which are assumed in \gls{HPSG}. See \textcite{Sag2010,Sag2012} for
discussion.} Types provide a complex classification of linguistic objects,
features identify their basic properties, and constraints impose further
restrictions. The central focus of \gls{HPSG}\is{Head-Driven Phrase Structure
Grammar} is signs.  For \citet{GinSag2000}, the type \emph{sign} has the
subtypes \emph{lexical-sign} and \emph{phrase}, and \emph{lexical-sign} has the
subtypes \emph{lexeme} and \emph{word}. Thus, we have the following type
hierarchy:
...
For example, \gls{HPSG}\is{Head-Driven Phrase Structure Grammar} has a feature HEAD, whose value is a \emph{part-of-speech},
...
\REF{ex:key:4.7}. It is complex, but each component of it has a purpose. The MOD
...
\z
%
The bracketed material in \REF{ex:key:4.9} is a \emph{wh}-relative,
\REF{ex:key:4.10} is a \emph{wh}-exclamative, and \REF{ex:key:4.11} is what has
...
an empty element. For \gls{HPSG}\is{Head-Driven Phrase Structure Grammar}, as indicated earlier, \isi{relative clauses} have a
...
directly with the value of SLASH, as in \REF{ex:key:4.14}:
...
\ea\label{ex:key:4.15}\ili{Welsh}
...
in \ili{Archi}. On this analysis, \REF{ex:key:4.24} has the schematic analysis in
\REF{ex:key:4.25}.
...
\ea\label{ex:key:4.26}\ili{Arabic}
...
\ea\label{ex:key:4.27}\ili{Arabic}
...
\ea\label{ex:key:4.28}\ili{Arabic}
...
\ea\label{ex:key:4.29}\ili{Arabic}
    \sn\gll laqad    qaabala    Aħmad-a.\\
            indeed met.\Tsg.\M{} Ahmad-\Acc{}\\
    \glt    \enquote*{He met Ahmad.}
\z
%
Essentially the same analysis can be applied to \REF{ex:key:4.26}. That is, it
too can be analysed as involving an initial \isi{topic} coindexed with a null
pronominal subject. If this is right, \REF{ex:key:4.26} is not a
...
different if they have either of the analyses in \REF{ex:key:4.22} and
\REF{ex:key:4.23}. (On the analysis in \REF{ex:key:4.23} the clause is a
...
\REF{ex:key:4.30} has the schematic analysis in \REF{ex:key:4.31}.

\ea\label{ex:key:4.30}\ili{Welsh}
...
\ea\label{ex:key:4.31}
    $\begin{bmatrix*}[l]
        ...
        $DOM$ & $\tuple{[\emph{gwelodd}], [\emph{Emrys]}, [\emph{y ddraig}]}$\\
    \end{bmatrix*}$
\z
%
...
and in favour of an analysis of the kind in \REF{ex:key:4.23}. It could be,
however, that the approach in \REF{ex:key:4.31} is appropriate for other VSO
...
\REF{ex:key:4.32}, which might have the analysis in \REF{ex:key:4.33}.
...
\ea\label{ex:key:4.33}
    $\begin{bmatrix*}[l]
        ...
        $DOM$ & $\tuple{[\emph{a man}], [\emph{came in]}, [\emph{who looked like Chomsky}]}$\\
    \end{bmatrix*}$
\z
%
...
\ea\label{ex:key:4.34}\ili{Arabic}
...
the gloss of \REF{ex:key:4.34b} makes clear, \emph{thakaʔ-an} is not an
...
\ea\label{ex:key:4.35}\ili{Arabic}
    \sn\gll ʔanaa        ʔakthar-u    maal-an      min    ali-in\\
            \Fsg.\M{} more-\Nom{} money.\Acc{} from Ali-\Gen{}\\
    \glt    \enquote*{I have more money than Ali.}
\z
%
It is fairly clear that \REF{ex:key:4.34b} involves the same comparative
construction as \REF{ex:key:4.35}. To reflect this, it could be translated as
\enquote{I have more intelligence than Ali}. The comparative construction in
\REF{ex:key:4.34b} and \REF{ex:key:4.35} is quite like what is called the
adjectival construct construction, illustrated in \REF{ex:key:4.36}.

\ea\label{ex:key:4.36}\ili{Arabic}
...
\REF{ex:key:4.34b} is very different from its counterpart in \ili{English} and
