...
\author{Nigel Vincent\lastand Kersti Börjars\affiliation{The University of Manchester}}
...
\emph{oti}; nouns as for instance \ili{Korean} \emph{k\textepsilon s} $<$
...
\citep[870--4]{KehaBoye16}. As we shall see in what follows, they may also
...
\label{Fig1LFGCats}
 \begin{tabular}{lll}
  ...
  \midrule
     ...
     \addlinespace[1ex] have \textsc{pred} feature &  no \textsc{pred} feature &  \enquote{full} semantics\\
     \addlinespace[1ex] projects to XP &   projects to XP &    does not project    \\ & \enquote{extension} of lexical category; & adjoins to X$^0$\\
      \addlinespace[1ex]    &  functional co-head & \\
  \lspbottomrule
 \end{tabular}
\end{table}
...
suggestion by \cite[44--6]{PollSag94} that such items belong to a separate
...
Grammar} and \gls{HPSG}.\largerpage[1]

\begin{table}
\caption{Types of category in HPSG}
\label{Fig2HPSGCats}
{\small
 \begin{tabular}{lll}
  \lsptoprule
  ...
  \midrule
     ...
     \addlinespace[1ex]\textsc{content} feature &  no \textsc{content} feature &  no \textsc{content} feature\\
     ...
     \addlinespace[1ex]    & the \textsc{content} feature & shares \textsc{head}\\
  \lspbottomrule
 \end{tabular}
 }
\end{table}
...
\begin{table}
\caption{Heads in \gls{LFG}\is{Lexical Functional Grammar} and \gls{HPSG}}
\label{Fig3CatsLFGHPSG}
{\small
\begin{tabular}{@{}l  ccc  ccc@{}}
  \lsptoprule
        &\multicolumn{3}{c }{LFG} & \multicolumn{3}{c}{HPSG}\\
     & {\small Lexical}  &    {\small Funct}  &  {\small Non-proj} &  {\small Full} & {\small Full\tss{\emph{transp}}} & {\small Weak}     \\
     \midrule
{\small lexical semantics} &  + &  -- &  +/-- & + & -- & -- \\
    {\small \enquote{borrows} lexical semantics} &  -- & + & -- & -- & + & +    \\
      {\small projects}   & + & + & -- & + & + & −\\
    {\small combines with} & XP & XP & X & XP & XP & XP (X$'$) \\
...
\begin{xlist}
\ex
\gll Hon nj\"ot av {\bf att} efter m\aa nga	\aa r \aa ter {\bf k\"anna} fast mark under f\"otterna.\\
...
\trans \enquote*{She says that he is going to come today.} (Faroese)
\end{xlist}	\end{exe}
...
Kayne's position, see \cite{Borsley01}.}\largerpage
...
\emph{de}, and if it is a finite clause we have \emph{que}, as in (\ref{ex17}).
...
grammar such as the Late \isi{Merge} and Economy principles proposed by
\cite{vanGelderen2009, vanGelderen11}, or to consider the driving force of change
to be the external circumstances of language use, but to deploy the devices of
formal syntax in order to model such changes as and when they are attested.
Thus, if, over time, we find evidence of nouns evolving into prepositions,
prepositions evolving into complementisers\is{complementizers} and prepositions
evolving from lexical (\enquote{full semantics}) to grammatical (\enquote{weak}
semantics), but we do not have any attested cases of the reverse,  we may
reasonably ask: why not? The answer, we suggest, lies in the fact that
non-finite forms start out as nominal and shift to verbal as they are
incorporated into the verbal paradigm. There is, by contrast, no corresponding
nominalisation of finite forms. In other words, the directionality follows from
the content and contextual function of the constructions at issue and does not
need to be ascribed to any principle of \gls{UG}.
