...
namely \ili{Italian} \emph{ben} (lit.\ ‘well’), as employed in a selection of regional
...
impatience, e.g.\ Dolomitic \ili{Ladin} \emph{po}, \citealt[52]{Hack2014}) or context
...
\ea\label{ex:key:12.1} \ili{Italian}
	\ea
...
\ea\label{ex:key:12.4} Group 1 / 3 (Italian)\\
	\ea[*]{
        \gll   Gianni vuole che Marco compri \textbf{ben} qualcosa per cena.\\
			    G. want.\Prs.\Tsg{} that M. buy.\Sbjv{}.\Tsg{} \textsc{ben} something for dinner\\
        \glt    \enquote*{Gianni wants Marco to buy something for dinner.}}\label{ex:key:12.4a}
	\ex[*]{
        \gll    Gianni avrebbe voluto che Marco avesse \textbf{ben} comprato
        qualcosa per cena.\\
		...
		\glt    \enquote*{Gianni would have wanted Marco to buy something for
                dinner.}}\label{ex:key:12.4b}
	\z
\z

\ea\label{ex:key:12.5} \ili{Italian}
...
\ea\label{ex:key:12.7}
    \ea (negative presupposition: the knight does not deserve to be treated in
        ...
        \textquote{Along the road they met King Meliadus, on his way to a tournament,
        also dressed as a knight errant and hiding his weapons. He asked the
        vassals: \enquote*{Why are you carrying this knight to the emperor? And who is
        he, that you are dishonouring him in such an uncivil manner?} The
        vassals replied: \enquote*{He well deserves to die; and if you knew why, you
    ...
    \ex (context: there is a quarrel involving Lena’s son and Pero Stomarin.
        ...
        \textquote{[\dots] she said that Çanun said that Lena’s son had taken
...
relationships described in (\ref{ex:key:12.12}) in terms of a parameter hierarchy.\is{parameter hierarchies} Following
...
\ea\label{ex:key:12.13} For a given value \emph{v\textsubscript{i}} of a parametrically variant feature F:
	\ea \textbf{Macroparameters}: all functional heads\is{functional items} of the relevant type share \emph{v\textsubscript{i}};
	\ex \textbf{Mesoparameters}: all functional heads\is{functional items} of a given naturally definable class, e.g.\ [+V], share \emph{v\textsubscript{i}};
	\ex \textbf{Microparameters}: a small subclass of functional heads\is{functional items} (e.g.\ modal auxiliaries) shows \emph{v\textsubscript{i}};
...
\ea\label{ex:key:12.15}
    \hspace*{-1.5em}\begin{tikzpicture}[baseline, font=\smaller]
