...
from \ili{English}, and (5)--(7), from other languages. In each case, different verbs
...
\ea \emph{Ne}-cliticisation (Italian)
    \ea[]{%
        ...
        \glt \enquote*{(Of them,) many arrived.}\hfill\parencite[221]{Bentley2004}}
    \ex[*]{%
        \gll Ne   studiano   molti.\\
            of-them study-\Tpl{} many-\M.\Pl{}\\
        \glt \enquote*{(Of them,) many studied.}\hfill\parencite[222]{Bentley2004}}
    \z
\z

\ea Case marking (\ili{Georgian})
    \ea
        \gll    Rezo   gamoizarda.\\
                Rezo.\Nom{}   he.grew.up\\
...
in~\sectref{sec:visco}.  In §3, I then compare the VISCO approach with previous
...
including particularly \ili{Basque} and \ili{Georgian}; see
...
work. §3.2 overviews the \isi{Unaccusative Hypothesis} as originally formulated. §3.3
...
\subsection{The origins of the Unaccusative\is{unaccusativity} Hypothesis}
...
\ea\label{ex:unacc}\textbf{The \isi{Unaccusative Hypothesis}:} \enquote{Certain
...
\ea \ili{Dutch}
    \sn[*]{\gll    Er   werd   gebloed.\\
            there   became   bled\\
    \glt    \enquote*{There was bleeding.}\hfill(adapted from \citealt[131]{Zaenen1993})\\}
\z
Perlmutter’s idea is that the impersonal \isi{passive} is possible with unergative
clauses, but not unaccusative\is{unaccusativity} ones (for details of the mechanics of this, see
...
internal argument in the complement position of VP:\largerpage[2]
...
\subsection{The central insight of the Unaccusative\is{unaccusativity} Hypothesis}
...
\emph{disappear} are meant to be unaccusative\is{unaccusativity}. What happens, then, when verbs
...
verbs meaning \enquote{to sweat} show unaccusative\is{unaccusativity} properties in Choctaw
...
\ea \ili{Italian} \parencite[62]{Rosen1984}\\
    \sn
    \gll Ho sudato.\\
    have.\Fsg{} sweated\\
    \enquote*{I sweated.}\\
...
\ea\label{ex:19.35} \ili{Dutch} (adapted from \citealt[140]{Zaenen1993})
    \ea[]{%
        \gll de   gevallen   jongen\\
             the   fallen   boy\\\hfill[$+$telic]
         \glt  \enquote*{the boy who has fallen}\\}
    \ex[*]{%
        \gll    de   gewerkt  man\\
             the   worked     man\\\hfill[$-$telic]
         \glt   \enquote*{the man who has worked}}
    \z
\z

\ea\label{ex:19.36} \ili{Dutch} (adapted from \citealt[131]{Zaenen1993})
    \ea[]{%
        \gll Er   werd   hard   gewerkt.\\
        there   became   hard   worked\\\hfill[$+$control]
        \glt \enquote*{There was hard work.}}
    \ex[*]{%
        \gll Er werd   gebloed.\\
            there   became   bled\\\hfill[$-$control]
...
prenominal past \isi{participles} but disallow resultatives:
...
\begin{exe}\exi{(i)} The butter melted into the toast.\end{exe}
\begin{exe}\exi{(ii)} Lucy was freezing away outside in the snow.\end{exe}
\begin{exe}\exi{(iii)} The play died a death.\end{exe}
\begin{exe}\exi{(iv)} survivor\end{exe}
...
\begin{exe}\exi{(i)} In the room remained a man.\end{exe}
\begin{exe}\exi{(ii)}There remained a man.\end{exe}
...
\ea \ili{Dutch} \parencite[870]{Sorace2000}
    \ea
        \gll    Het   beeldje \textbf{heft} op   de   tafel   gestaan.\\
...
whether \ili{English} stative verbs are unergative\is{unergativity} or unaccusative? They have
...
\begin{exe}\exi{(i)}[]{The wood blackened.}\end{exe}
\begin{exe}\exi{(ii)}[]{The fire blackened the wood.}\end{exe}
\begin{exe}\exi{(iii)}[*]{The wood blackened black.}\end{exe}
...
is most easily illustrated with auxiliary\is{auxiliaries} selection, which is also the
phenomenon best studied with relation to the SIH\@. The generalisation which
can be made is that in languages with a HAVE/BE split amongst auxiliaries in
the periphrastic perfect with intransitives, BE is associated with heads below
a certain point when they bear a positively valued feature (e.g.\ with Oriented
when it bears [$+$oriented], but not where it bears [$-$oriented]); HAVE is
then associated with heads bearing a positively valued feature above that
point. The cut-off point in question, however, varies between
...
corresponding positions, as discussed above.)\largerpage[1]
...
\z
This allows the learner to posit InitiationP as higher in the structure than
ChangeP. See \citet{Baker2018} for in-depth discussion.
