...
the verbal (eventive) passive\is{passive} (e.g., \emph{sold} as in ‘The first costumer was
...
\ea\label{ex-14:key:1} \ili{Hebrew}
    \ea Unaccusative
    \sn[]{\gll    yaca  le-\emph{x} me-ha-af \\
                went.out to-\emph{x} from-the-nose \\
            \glt    \enquote*{got tired of}}
    \ex Transitive
    \sn[\#]{\gll hoci   le-\emph{x} me-ha-af \\
    took.out to-\emph{x} from-the-nose \\}
    \z
\z

\ea\label{ex:key:20.2} \ili{Hebrew}
...
\REF{ex:key:20.4}, and illustrated in (\ref{ex-14:key:1})--\eqref{ex:key:20.3} and \REF{ex:key:20.5}
...
\ea\label{ex:key:20.4}Phrasal versus clausal idioms
    \ea Phrasal \isi{idioms} are headed by a lexical head (e.g., (\ref{ex-14:key:1})--\eqref{ex:key:20.3}).
    ...
    \REF{ex:key:20.5}, where the modal and negation are
...
In~\Cref{sec:key:20.2} we report a survey of four \ili{English} idiom\is{idioms} dictionaries,
...
In \Cref{sec:key:20.3} we describe an experiment which tested the likelihood of
...
transitive alternant. \Cref{sec:key:20.4} offers our analysis of the findings
in terms of lexical storage, and \Cref{sec:key:20.5} evaluates possible
...
looking for ``unique'' \isi{idioms} (as defined in \REF{ex:key:20.6}).\footnote{The
...
\ea\label{ex:key:20.6} Uniqueness\\
    An idiom\is{idioms} is ``unique'' to a given diathesis α, if α does not share the idiom
    with its (existing) root-counterpart in diathesis β, which α would most
    directly be related to by derivation.  Specifically,
    \ea An idiom\is{idioms} in the verbal passive\is{passive} / adjectival passive\is{passive} / anticausative is
...
\ea\label{ex:key:20.7}
    \ea[]{The agreement seems written in stone / \#is being written in stone.}
    \ex[\#]{The agreement was written in stone in a few hours / to make
    people respect it.}
    \z
\z
...
\ea\label{ex:key:20.8}
    \ea[]{The beans were being spilled / \#seem spilled.}
    \ex[]{The beans were spilled in a few minutes / in order to attract
        attention.}
    \z
\z
...
As shown in \Cref{tab:key:20.1} below, transitives, anticausatives and
adjectival passives exhibited unique \isi{idioms}, just like their \ili{Hebrew}
counterparts. Examples of unique anticausative \REF{ex:key:20.9}, adjectival
passive \REF{ex:key:20.10}, and transitive \REF{ex:key:20.11} \isi{idioms} are given below.
...
unique \isi{idioms} that they occur in are given in \ref{app-14:a}.
...
\ea\label{ex:key:20.9}
    \ea[]{Anticausative}
    \sn[]{burst at the seams\\
        ‘filled (almost) beyond capacity’}
    \ex[]{Transitive}
    \sn[\#]{burst something/someone at the seams}
    \z
\z
\newpage
...
\emph{prima facie}, to present unique verbal passive\is{passive} \isi{idioms}
for two out of the 60
predicates, namely for \emph{caught} and \emph{bitten}. These \isi{idioms} are given
in \REF{ex:key:20.12}. The combination \emph{the x bug} is instantiated by
...
are two online examples of the \isi{idioms} in (\ref{ex:key:20.12}a,b) in the
verbal passive\is{passive}.

\begin{exe}

    \exi{(i)} \dots{} parents and staff were concerned that faith schools were
    being “caught in the crossfire” between Ofsted and the Department
    \dots{}\\\hspace*{\fill}\url{https://goo.gl/jqwnDD}

    \exi{(ii)} During one of these journeys, I was firmly bitten by the travel
...
verbal passive\is{passive}. Starting with \REF{ex:key:20.12a}, the idiom\is{idioms} \emph{caught}
...
(active) form, as in \REF{ex:key:20.13}, for instance.
...
As for the idiom\is{idioms} in \REF{ex:key:20.12b}, again, Google searches turn up a
significant number of active transitive examples (e.g.,
\REF{ex:key:20.14}, \eqref{ex:key:20.15}).
...
The listing of (\ref{ex:key:20.12}a,b) in the passive\is{passive} participial form may well
...
%    \ex The Starbucks coffee chain remains \textbf{caught in the crossfire} of
...
%    \ea \dots{} and Kevin remains \textbf{bitten by the travel} -- and mapping
%    -- \textbf{bug}.\\\hspace*{\fill}\url{https://goo.gl/xC8kWp}
%    \ex
...
We can thus conclude that the \isi{idioms} in \REF{ex:key:20.12} are not exceptions to
...
\Cref{tab:key:20.2}. To evaluate the significance of the relationship between
...
set (as in \Cref{tab:key:20.2} and \Cref{fig:key:20.1} below) and once on a
data set consisting of the transitives, anticausatives and adjectival passives
(excluding the verbal passives, as in \Cref{tab:key:20.1} above).

\begin{table}
\caption{Distribution of anticausative, adjectival passive\is{passive},
transitive and verbal passive\is{passive} in unique idioms}\label{tab:key:20.2}
...
unique \isi{idioms} (χ² = 19.7, \emph{p} < .001, corrected), the one performed on all
...
\largerpage
\paragraph*{Stimuli} We composed 12 \ili{English} \isi{idioms} inspired by existing
...
verbal.\footnote{\textcite{LevRap1986} put forward the \enquote{\glsdesc{SCG}}
    ...
    \begin{exe}
        ...
        \exi{(i)}[]{They sold the car.}
        \exi{(ii)}[]{The sold car.}
        \exi{(iii)}[*]{They sold the guy. (intended meaning: \enquote*{They
        sold the guy something.})}
        \exi{(iv)}[*]{the sold guy (intended meaning: \enquote*{the guy who was
        sold something})}
        \exi{(v)}[]{\dots{} was sold something}

    \end{exe}} The full list of invented \isi{idioms}, including their \ili{Hebrew}
...
session, \isi{idioms} were taught based on a list of \isi{idioms} including their
...
in the trash can} \REF{ex:key:20.18a} was associated with the interpretation in
\REF{ex:key:20.18b} and usage example in \REF{ex:key:20.19}.  The comprehension
task, the completion task and the experimental task for this idiom\is{idioms} are
given in \REF{ex:key:20.20}, \REF{ex:key:20.21}, and \REF{ex:key:20.22},
...
For all the \isi{idioms} used in the experiment see \ref{app-14:b} (Form 1).
...
\ref{app-14:b}), and taught the \isi{idioms} by reading each idiom\is{idioms} aloud in various
...
shown in~\Cref{fig:key:20.2} below.
...
\largerpage[2]
As shown in \Cref{tab:key:20.3} below, planned pairwise comparisons with an
...
\section{Discussion}\label{sec:key:20.4} %4. /
...
\section{Alternative Approaches}\label{sec:key:20.5} %4.2 /
...
(anticausative) alternation – certain unaccusative\is{unaccusativity} verbs idiosyncratically lack
a transitive counterpart in a given language, and vice versa – anticausative
and transitive \isi{idioms} can similarly lack the relevant alternant. Indeed, in the
case of the verbal passive\is{passive}, an approach of inheritance from the verb to the
idiom level can be envisioned. However, as far as the anticausative alternation
is concerned, this approach is implausible. While uniqueness at the idiom\is{idioms} level
is a pervasive phenomenon (as shown in \Cref{tab:key:20.2} and exemplified in
(\ref{ex-14:key:1})--\eqref{ex:key:20.3} and
(\ref{ex:key:20.9})--\eqref{ex:key:20.11} above), unaccusative\is{unaccusativity} verbs systematically have a
...
existing \isi{idioms} in both \ili{English} (\Cref{sec:key:20.2}) and \ili{Hebrew}
...
head responsible for the formation of verbal passives. The verbal passive\is{passive} would
not give rise to unique \isi{idioms} because it would be beyond the syntactic domain
of special meanings. Such a proposal would be at odds with \citegen{Arad2005}
...
domain, trying to cover the split behavior of \isi{idioms} in the verbal
passive versus the other diatheses we examined, seems ad hoc.\footnote{Under
    ...
    \citealt{Harley2013}), the passive\is{passive} Voice head merely captures the absence
    ...
    unique verbal passive\is{passive} \isi{idioms} to the Voice head’s lack of semantic
...
ability to account for the idiom\is{idioms} data. }
...
adjectival passive\is{passive} \isi{idioms}, \ref{app-14:a}).\largerpage[1]
...
%\begin{table}[htp]
%    \caption{Sampled predicates (continued)}
{\smaller
\begin{tabularx}{\textwidth}{XXXX}
%\lsptoprule
{\bfseries Transitive}  & {\bfseries Unaccusative} & {\bfseries Adjectival passive} & {\bfseries Verbal passive}\\
...
\paragraph*{Unique unaccusative\is{unaccusativity} idioms}\leavevmode\\[.5\baselineskip]
{\smaller
...
%\begin{table}[htp]
\paragraph*{Unique adjectival passive\is{passive} idioms}\leavevmode\\[.5\baselineskip]
{\smaller
\begin{tabularx}{\textwidth}{lXX}
%\lsptoprule
...
\largerpage[2]
\paragraph*{Form 1} Idioms\is{idioms}, interpretations and usage examples (the
copies distributed to participants did not include indications of
diatheses)\leavevmode\\[.5\baselineskip]

%\begin{table}[htp]
%\caption*{Form 1: \isi{idioms}, interpretations and usage examples (the copies
%    distributed to participants did not include indications of diatheses)}
\noindent{\smaller
    \begin{tabularx}{\textwidth}{@{}lXp{2cm}p{5.5cm}@{}}
%\lsptoprule
& {\bfseries Idiom} & {\bfseries Interpretation}  & {\bfseries Usage example} \\
\midrule
\textbf{1.} & \textbf{was sold a stew of lentils} \newline
            (verbal passive) \newline
...
%\textbf{2.} & \textbf{stained under his/her skin} \newline
%            (adjectival passive) \newline
%            Inspiring \ili{Hebrew} idiom: \newline
...
%            from one's knowledge & The first person Lisa thanked in the acknowledgments part of her dissertation was her thesis advisor Professor Green. After all, Lisa has warmed up in his light ever since she was an undergrad.\\
%\lspbottomrule
\end{tabularx}
}
%\end{table}
...
\textbf{6.} & \textbf{was handed soggy bread} \newline
            (verbal passive) \newline
            Inspiring \ili{Hebrew} idiom: \newline
...
%\textbf{7.} & \textbf{drown in the trash can} \newline
%            (unaccusative) \newline
%            Inspiring \ili{Hebrew} idiom: \newline
...
\textbf{7.} & \textbf{drown in the trash can} \newline
            ...
            fell in.the-bin\newline
            \enquote*{got fooled} & get fooled & Alice really enjoys playing practical jokes on her friends and family. They are all pretty gullible, but her favorite victim is her little sister who somehow manages to drown in the trash can each and every time Alice tries to set her up. \\
\textbf{8.} & \textbf{break out of his/her harness} \newline
            (unaccusative) \newline
            Inspiring \ili{Hebrew} idiom: \newline
...
\paragraph*{Form 2} Comprehension questions%\leavevmode\\[.5\baselineskip]
{\small
...
\noindent Native \ili{English} speaker: yes/no
...
\begin{exe}
    ...
    \begin{xlist}
        \exi{1.} his heart broke
        \exi{2.} he was so depressed that he sat in the dark with a flashlight
        \exi{3.} his suspicion arose
    \end{xlist}
    \end{xlist}
    \exi{2.}
    \begin{xlist}
    ...
    \exi{3.}
        ...
        \begin{xlist}
        \exi{1.} she was given the wrong bowl of soup
        \exi{2.} she was deceived
        \exi{3.} she received confidential news
        \end{xlist}
        \end{xlist}
    \newpage
    \exi{4.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} she was blamed for something she didn't do
        \exi{2.} she was told a fib in order to keep her quiet
        \exi{3.} she was yelled at by angry customers
        \end{xlist}
        \end{xlist}
    \exi{5.} \begin{xlist}
    ...
    \exi{6.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} she is embarrassed
        \exi{2.} she is left with nothing
        \exi{3.} she is bruised
        \end{xlist}
        \end{xlist}
    \exi{7.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} she was soiled by a splash of mud
        \exi{2.} she was asked for a loan
        \exi{3.} she was maligned behind her back
        \end{xlist}
        \end{xlist}
    \newpage
    \exi{8.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} he was injured by bumping into a beam
        \exi{2.} he was held responsible for the failure
        \exi{3.} he had to solve the problem
        \end{xlist}
        \end{xlist}
    \exi{9.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} he has benefited from her knowledge
        \exi{2.} he has enjoyed her warm office
        \exi{3.} he has felt comfortable around her
        \end{xlist}
        \end{xlist}
    \exi{10.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} she is lazy
        \exi{2.} she is trapped in a comfortable but damaging situation
        \exi{3.} she is not a vegetarian
        \end{xlist}
        \end{xlist}
    \exi{11.} \begin{xlist}
    ...
    \exi{12.} \begin{xlist}
        ...
        \begin{xlist}
        \exi{1.} she got so excited she walked right into a bin full of garbage
        \exi{2.} she got  fooled
        \exi{3.} she got upset
        \end{xlist}
        \end{xlist}
\end{exe}
...
\begin{exe}
%    \sn \textbf{Form 3}: Completion task
    \sn
    \begin{enumerate}
...
\noindent 1.\ You have learned the idiom \enquote*{\textbf{break out of his/her harness}}.
...
\noindent 2.\ You have learned the idiom \enquote*{\textbf{was slipped the smudge}}.
...
\noindent 3.\ You have learned the idiom \enquote*{\textbf{drown in the trash can}}.
...
\noindent 4.\ You have learned the idiom \enquote*{\textbf{shaven on both sides of
    her/his face}}.
...
\noindent 5.\ You have learned the idiom \enquote*{\textbf{one's red bulb lit up}}.
...
\noindent 6.\ You have learned the idiom \enquote*{\textbf{sunken in a pit of lard}}.
...
\noindent 7.\ You have learned the idiom \enquote*{\textbf{was given the heavy beam}}.
...
\noindent 8.\ You have learned the idiom \enquote*{\textbf{warm up in one's light}}.
...
\noindent 9.\ You have learned the idiom \enquote*{\textbf{was sold a stew of lentils}}.
...
\noindent 10.\ You have learned the idiom \enquote*{\textbf{stained under his/her
    skin}}.
...
\noindent 11.\ You have learned the idiom \enquote*{\textbf{was fed soggy bread}}.
...
\noindent 12.\ You have learned the idiom \enquote*{\textbf{stricken with tremors}}.
