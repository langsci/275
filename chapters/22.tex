...
derivational option is independently excluded,\footnote{For example, the Phase
    ...
    where this would cross a phase boundary. See~\Cref{sub:key:22.2.2}
below.\label{fn:22.2}} we might expect the opposite pattern to obtain. These
two scenarios were summarized in \citet{BibRich2006} as in \REF{ex:key:22.1}.

\ea\label{ex:key:22.1}
    \ea\label{ex:key:22.1a} Optional operations feed obligatory interpretations;
    \ex\label{ex:key:22.1b} Obligatory operations feed optional interpretations.
    \z
\z
...
therefore result (i.e.\ a lack of \gls{EOO}, equivalent to \REF{ex:key:22.1b}).
Let us refer to this as Claim 1, as in \REF{ex:key:22.2}.

\ea\label{ex:key:22.2}\textbf{Claim 1}\\
    The phase\is{phases} is the maximal domain in which syntactic/interpretive constraints
    can apply. Each choice within a phase\is{phases} registers a distinct \gls{EOO} at the
    interface.
\z

Effectively, the \gls{EOO} rationale in \REF{ex:key:22.1a}, in combination with
...
\REF{ex:key:22.3} ostensibly confirms.
...
in~\Cref{sub:key:22.2.1}. This then leads to my main claim,
in~\Cref{sub:key:22.2.2} --- namely, that \isi{binding} conditions (Principles A and
...
naturalize the \isi{binding} principles in terms of \REF{ex:key:22.2} and
\REF{ex:key:22.4}, i.e.\ as emergent \glspl{EOO}, we will have gone some way towards
...
\REF{ex:key:22.2} and \REF{ex:key:22.4} conspire to constrain the interpretive output
...
above the object, as in \REF{ex:key:22.5b}.

\ea\label{ex:key:22.5} \ili{Icelandic}
    \ea\label{ex:key:22.5a}
        \gll    Nemandinn  las   [\emph{\tss{v}}\tss{*P} (\textbf{bókina}) \emph{t}\tss{nemandinn} ekki [\tss{VP} \emph{t}\tss{las} (\textbf{bókina}) ]] \\
    ...
    \ex\label{ex:key:22.5b}
        \gll    Nemandinn hefur [\emph{\tss{v}}\tss{*P} (\textbf{*bókina}) \emph{t}\tss{nemandinn} ekki [\tss{VP} lesið (\textbf{bókina}) ]]\\
...
\REF{ex:key:22.2} and \REF{ex:key:22.4} above. An alternative is offered in
...
the phase\is{phases} but not within, in conformance with \REF{ex:key:22.2} and
\REF{ex:key:22.4}. This alternative follows from a Merge-based linearization
...
linearization may be expressed as in \REF{ex:key:22.7}.
...
The contrast between \REF{ex:key:22.5} and \REF{ex:key:22.6} is a
straightforward consequence of this system. As depicted in \REF{ex:key:22.8},
short object displacement to spec-\emph{v}P across V is only orderable by
\REF{ex:key:22.7a} where further \isi{movement} of V across the displaced object
...
\REF{ex:key:22.7a}: VO languages). Beyond the \emph{v}P phase\is{phases} level, however,
...
\REF{ex:key:22.6}). Interpretive freedom at \gls{PF} is thus the result of
...
\REF{ex:key:22.5}-(6) in which \isi{Merge} choices made locally (within the
...
based on \REF{ex:key:22.2} and \REF{ex:key:22.4}.
...
obviation. These core facts are given in \REF{ex:key:22.9}.

\ea\label{ex:key:22.9}
    \ea\label{ex:key:22.9a} \makebox[0pt][l]{He\tss{i} likes himself\tss{i/*j}}%
    ...
    \ex\label{ex:key:22.9b} \makebox[0pt][l]{He\tss{i} likes him\tss{*i/j}}%
        \phantom{He thinks that Mary likes him\qquad}%
        (Principle B\is{binding}, local; obligatory \hphantom{He thinks that Mary likes
        him\qquad} obviation/disjoint reference)
    \ex\label{ex:key:22.9c} \makebox[0pt][l]{He\tss{i} thinks that Mary likes him\tss{i/j}}%
        \phantom{He thinks that Mary likes him\qquad}%
        (non-local; referential freedom)
    \ex\label{ex:key:22.9d} \makebox[0pt][l]{His\tss{i} mother likes him\tss{i/j}}%
        \phantom{He thinks that Mary likes him\qquad}%
...
To derive the contrast between \REF{ex:key:22.9a} and \REF{ex:key:22.9b},
...
different \gls{EOO} (cf.\ \REF{ex:key:22.2}).

According to the first option, the \gls{IA} may be raised to spec-\emph{v}P to
form the \gls{EA}, as in \REF{ex:key:22.10}.\footnote{The lower copy here is
    ...
    \REF{ex:key:22.10} involves a transitive \emph{v}*, which values Case
...
CI-interface (cf.\ \REF{ex:key:22.2}). The two occurrences of the relevant lexical
...
result (\glsunset{EOO}\gls{EOO}) is obligatory disjoint reference at SEM (i.e.\ \emph{he} ${\neq}$
...
Turning finally to \REF{ex:key:22.9c} and \REF{ex:key:22.9d}, here the two indexed
positions cannot be derivationally related by \gls{IM}\is{Merge}. In the case of
\REF{ex:key:22.9c}, this is due to the presence of at least one intervening phase
...
Impenetrability Condition. In \REF{ex:key:22.9d}, an interarboreal or sideward
...
\emph{him}. In both cases, therefore, only \gls{EM}\is{Merge} is possible.\footnote{The
    ...
    \REF{ex:key:22.10}/(11) above), it will be associated with interpretive
    freedom, in line with \REF{ex:key:22.1b}.  Consequently, incidental
    coreference/covariance becomes a possible interpretation. As with the
    trans-phasal dependencies in \REF{ex:key:22.6}, crossing a phase\is{phases} results in
    ...
    either via the phase-internal, obligatory route (cf.\ \REF{ex:key:22.9a}), or
    ...
    research.\footnote{\citet{Hornstein2013} independently argues for a
        ...
        \REF{ex:key:22.9d}-type configurations, as in \emph{Everyone\tss{i}’s
        ...
        instantiating \REF{ex:key:22.9c}, where again the bound reading is only
        optional.  As discussed in~\Cref{sec:key:22.1}, there is no need for
...
due to the \gls{EOO} rationale in \REF{ex:key:22.1}/(2).  The \gls{MTB} in
...
structures like \REF{ex:key:22.3}; non-local SE anaphors (contrasting with
...
\REF{ex:key:22.9c}). An approach in terms of cancellation or extension of the
...
(cf.~\Cref{sec:key:22.1}; indeed, its main conceptual advantage is that it only
...
non-complementarity should re-emerge, at either or both interfaces. For SEM, an
example of such non-complementarity has already been discussed (the freely
interpreted embedded pronoun in \REF{ex:key:22.9c}); the PHON equivalent (i.e.\
