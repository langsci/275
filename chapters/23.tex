...
\abstract{Chinese has been widely recognised as a classic example of a
    ...
    data from Mandarin\il{Mandarin} and \ili{Hong Kong Cantonese} to show that the need of
    ...
    animacy. The findings present a consistent pattern that \ili{Hong Kong Cantonese}
    has a stricter requirement for classifiers in enumeration as bare common
    nouns are not definite in \ili{Hong Kong Cantonese}, and it lacks the alternative
...
\ea\label{ex:key:23.1}Mandarin \parencites[92]{Chierchia1998}[519]{ChengSybesma1999} %1
	\ea
...
\ea\label{ex:key:23.2}\gls{HKC} \parencites[14]{Sio2006}[272]{ChengSybesma2005}%2
	\ea
	\gll saam1 *(bun2) syu1\\
...
\ea\label{ex:key:23.3} \ili{Mi'gmaq} %3
    \ea[]{
...
\ea\label{ex:key:23.4} \ili{Mi'gmaq} %4
    \ea[*]{%
...
nouns criteria in \REF{ex:key:23.5}.

\ea\label{ex:key:23.5} Mass properties of Chinese nouns \citep[94]{Chierchia1998} %5
...
(\ref{ex:key:23.5}b) is too strong to hold. Turning back to Krifka’s alternative,
...
three.  \Cref{tab:key:23.1} has the list of \isi{numerals} tested; these are
...
considered are presented in \tabref{ex:key:23.2}. They vary in terms of degree
...
\ea\label{ex:23.6} Mandarin\il{Mandarin} %6
	...
	\ex
        \gll    wǒ yāoqǐng-le \textbf{yī-bǎi} \textsuperscript{?/*}\textbf{(ge)} \textbf{xuesheng}\\
                I  invite-\Pfv{} \textbf{one-hundred} \hphantom{\textsuperscript{?/*}(}\textbf{\Clf} \textbf{student}\\
	    \glt    ‘I invited one hundred students.’
	\z
\z

\ea\label{ex:23.7} \gls{HKC}\il{Hong Kong Cantonese} %7
    \ea[]{
        \gll    \textbf{saap6} \textbf{*(po1)} \textbf{syu6} sei2-zo2\\
                \textbf{ten} \hphantom{\textbf{*(}}\textbf{\Clf} \textbf{tree} die-\Pfv{}\\
        \glt    ‘Ten trees died.’}
    \ex[]{
        \gll    ngo5 maai5-zo2 \textbf{ji6-saap6-jat1} \textbf{*(bun2)} \textbf{zi6din2}\\
                I buy-\Pfv{} \textbf{two-ten-one} \hphantom{\textbf{*(}}\textbf{\Clf} \textbf{dictionary}\\
...
$\varnothing$ + \textsc{n}] structure, regardless of the value of the numeral\is{numerals}. However,
...
$\varnothing$ + N] which is shown to be sensitive to \isi{animacy} and number
...
cannot co-occur with a classifier\is{classifiers}, as in \eqref{ex:key:23.8}.

\ea\label{ex:key:23.8} %8
	\ea Mandarin
    \sn[*]{%
    \gll wǔ jīn kē cài\\
...
\ea\label{ex:key:23.9} Mandarin\il{Mandarin} %9
    \sn
...
The difference between Mandarin\il{Mandarin} and \gls{HKC}\il{Hong Kong Cantonese} in permitting [\Num{} +
...
\Cref{tb:key:3,tb:key:4} summarise the Mandarin\il{Mandarin} and \gls{HKC}\il{Hong Kong Cantonese} classifier
paradigms.
...
%        \node [fit=(gedan) (dprime) (cake), draw, very thick, inner sep=1mm] {};
    ...
    \caption{Mandarin classifier\is{classifiers} paradigm}\label{tb:key:3}
\end{table}
...
%        \node [fit=(gedan) (N) (cake), draw, very thick, inner sep=1mm] {};
    ...
    \caption{\gls{HKC} classifier\is{classifiers} paradigm}\label{tb:key:4}
\end{table}
...
(\emph{mat6fong1} \enquote*{bee} in \Cref{tb:key:4}), except when they act as
proper names (\emph{lou5ban2} \enquote*{boss} in \Cref{tb:key:4}).  Therefore,
...
Cantonese} more obligatorily requires the presence of a\largerpage[1]
...
classifier\is{classifiers} phrase (\Clf{}P) is projected whenever a definite
...
\ea\label{ex:key:23.12} Mandarin\il{Mandarin} %12
	\ea
...
\ea\label{ex:key:23.13} Mandarin\il{Mandarin} %13
    \ea[]{%
	    \gll    xuésheng-\textbf{men} xǐhuan chī miàn\\
	    	    student-\textbf{\Pl} like  eat  noodles\\
        \glt    ‘The students like to eat noodles.’}
    \ex[]{%
	    \gll    shí-èr \textbf{gè} xuésheng xǐhuan chī miàn\\
                ten-two \textbf{\Clf} student  like  eat  noodles\\
        \glt    ‘Twelve students like to eat noodles.’}
    \ex[*]{
        \gll    shí-èr \textbf{gè} xuésheng\textbf{-men} xǐhuan chī miàn\\
	    	    ten-two  \textbf{\Clf}  student-\textbf{\Pl}  like  eat  noodles\\
